...
* `secret_leak`: Flags leaked secrets/PII (AWS keys, JWTs, OpenAI/Anthropic keys, PEM private keys, emails, Luhn-valid credit cards, IPv4 addresses). Useful for scoring data-exfiltration objectives. Scans the high-signal categories by default (`email` and `ip` are opt-in); a subset can be selected via `judge_args` or `--judge-options`, and `all` enables every category.
